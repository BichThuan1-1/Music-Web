				...
				<div class="row">
					<div class="col-md-offset-3 col-md-6">
						...
						<section class="description">
							<div class="media">
								<div class="media-left media-middle">
									<img class="media-object img-circle" src="<%=Constant.ALBUM_PATH %>/${requestScope.songInfo.album.albumPic}" style="width:150px"/>
								</div>
								<div class="media-body">
									<h1 class="theme-color"><span class="fa fa-music"></span>&nbsp;<c:out value="${requestScope.songInfo.song.songTitle }"></c:out></h1>
									<div class="music-info">Ca sĩ：<span class="singer"><a href="<%=contextPath %>/SingerServlet?info=play&playId=${requestScope.songInfo.singer.singerId}"><c:out value="${requestScope.songInfo.singer.singerName }"></c:out></a></span></div>
								<div class="music-info">Album：<span class="album"><a href="<%=contextPath %>/AlbumServlet?info=play&playId=${requestScope.songInfo.album.albumId}"><c:out value="${requestScope.songInfo.album.albumTitle }"></c:out></a></span></div>
								</div>
							</div>
						</section>
						<!-- kết thúc phần mô tả bài hát-->
						<!-- thanh công cụ -->
						<section class="toolbar">
							<div class="btn-group" role="group">
								<button type="button" id="play" class="btn btn-primary"><span class="fa fa-headphones"></span>&nbsp;Nghe thử</button>
								<c:if test="${sessionScope.login_flag==login_success}">
									<a href="SongServlet?info=follow&songId=${requestScope.songInfo.song.songId }&userId=${sessionScope.user.userId }" type="button" class="btn btn-default"><span class="fa fa-heart-o"></span>&nbsp;Yêu Thích</a>
								</c:if>
								<a href="SongServlet?info=download&songId=${requestScope.songInfo.song.songId }&path=<%=Constant.MUSIC_PATH%>${requestScope.songInfo.song.songFile }" target="_blank" type="button" class="btn btn-default"><span class="fa fa-download"></span>&nbsp;Tải xuống</a>
								<c:if test="${sessionScope.login_flag==login_success}">
									<button type="button" data-toggle="modal" data-target="#commentModal" class="btn btn-default"><span class="fa fa-commenting-o"></span>&nbsp;Bình luận</button>
								</c:if>
							</div>
						</section>
						...
						<section class="comments" id="comment">
							<h2 class="theme-color">Bình luận</h2>
							<div class="comments-main">
								<ul class="media-list">
									<c:forEach items="${requestScope.songInfo.comments }" var="comment">
										<li class="media">
											<div class="media-left">
												<img class="media-object" src="<%=Constant.DEFAULT_AVATAR_SECPATH %>${comment.normalUser.userAvatar}" style="width:35px; object-fit:cover; border-radius: 20px; "/>
											</div>
											<div class="media-body">
											...
											<c:if test="${sessionScope.admin_login_flag==login_success}">
												<a href="#" class="pull-right">Xóa bình luận</a>
											</c:if>
										</li>
									</c:forEach>
								</ul>
